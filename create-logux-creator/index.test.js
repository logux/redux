...
it('does not fall on missed onMissedHistory', async () => {
  let store = createStore(historyLine)
  await store.dispatch.crossTab(
    { type: 'ADD', value: 'a' }, { reasons: ['first'] }
  )
  await store.log.removeReason('first')
  store.dispatch.crossTab(
    { type: 'ADD', value: '|' }, { id: '0 10:test1 0', reasons: ['test'] }
  )
  await delay(1)
...
it('cleans sync action after processing', async () => {
  ...
  store.dispatch.sync({ type: 'A' }).then(() => {
    resultA = 'processed'
  }).catch(e => {
  ...
  // eslint-disable-next-line jest/valid-expect-in-promise
  store.dispatch.sync({ type: 'B' }, { id: '3 10:1:1 0' }).then(() => {
    resultB = 'processed'
  }).catch(e => {
    expect(e.message).toContain('undid')
    expect(e.message).toContain('because of error')
...
it('supports middlewares', () => {
  let store = createStore(historyLine, { }, applyMiddleware(() => {
    return dispatch => {
      return action => {
        if (action.value !== 'a') {
          dispatch(action)
...
it('waits for replaying', async () => {
  ...
  await store.dispatch.crossTab(
    { type: 'ADD', value: 'b' }, { reasons: ['t'] }
  )
  await store.dispatch.crossTab(
    { type: 'ADD', value: 'a' }, { id: '0 test 0', reasons: ['t'] }
  )
  await Promise.all([
